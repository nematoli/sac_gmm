...
def dictlist_to_tensor(x, device):
    ...
    return OrderedDict([(k, torch.tensor(np.stack(v), dtype=torch.float32).to(device)) for k, v in x.items()])
...
def scale_dict_tensor(tensor, scalar):
    if isinstance(tensor, dict):
        return OrderedDict([(k, scale_dict_tensor(tensor[k], scalar)) for k in tensor.keys()])
...
def space2tensor(batch_size, space, scalar, device):
    ...
    if isinstance(space, gym.spaces.Dict):
        return OrderedDict([(k, space2tensor(batch_size, s, scalar, device)) for k, s in space.spaces.items()])
...
class RandomShiftsAug(nn.Module):
    ...
    def forward(self, x):
        ...
        arange = torch.linspace(-1.0 + eps, 1.0 - eps, h + 2 * self._pad, device=x.device, dtype=x.dtype)[:h]
        arange = arange.unsqueeze(0).repeat(h, 1).unsqueeze(2)
        base_grid = torch.cat([arange, arange.transpose(1, 0)], dim=2)
        base_grid = base_grid.unsqueeze(0).repeat(n, 1, 1, 1)
        shift = torch.randint(0, 2 * self._pad + 1, size=(n, 1, 1, 2), device=x.device, dtype=x.dtype)
...
class AdamAMP(object):
    ...
    def __init__(self, model, lr, eps, weight_decay, grad_clip, device, use_amp):
        self._model = model
        self._optim = optim.Adam(self._params(), lr=lr, eps=eps, weight_decay=weight_decay)
    ...
    def step(self, loss, retain_graph=False):
        ...
        grad_norm = nn.utils.clip_grad_norm_(self._params(), self._grad_clip)
...
def check_memory_kill_switch(avail_thresh=10.0):
    ...
    try:
        ...
        if stat.available * 100 / stat.total < avail_thresh:
            print(f"Current memory usage of {stat.percent}% surpasses threshold, killing program...")
