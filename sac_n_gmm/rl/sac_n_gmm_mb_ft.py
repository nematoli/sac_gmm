...
from sac_n_gmm.rl.task_mb_rl import TaskMBRL
import wandb
...
class SACNGMM_MB_FT(TaskMBRL):
    ...
    def __init__(
        ...
        self.agent.populate_replay_buffer_with_critic(self.actor, self.model, self.critic, self.replay_buffer)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % self.model.cfg.model_target_update_freq == 0:
    ...
    def evaluation_step(self):
        ...
        eval_accuracy, eval_return, eval_length, eval_skill_ids, eval_video_paths = self.agent.evaluate(
            self.actor, self.model, self.critic
        )
        eval_metrics = {
    ...
    def compute_model_loss(self, batch, model_optimizer):
        ...
        with torch.no_grad():
            skill_vector = self.agent.get_skill_vector(batch_skill_ids)
            model_input = torch.cat((enc_state, skill_vector), dim=-1)
        next_pred, batch_reward_pred = self.model.imagine_step(model_input.float(), batch_actions.float())
        with torch.no_grad():
            next_enc_state = self.model.encoder({"obs": batch_next_obs[OBS_KEY].float()})
            next_skill_vector = self.agent.get_skill_vector(batch_next_skill_ids)
            next_gt = torch.cat((next_enc_state, next_skill_vector), dim=-1)
        consistency_loss = torch.nn.MSELoss(reduction="none")(next_pred.float(), next_gt.float()).mean()

        # Reward Loss
        reward_loss = torch.nn.MSELoss(reduction="none")(
            batch_reward_pred.float(), batch_rewards.squeeze().float()
        ).mean()
    ...
    def load_checkpoint(self, model_ckpt, root_dir):
        ...
        # Get only model_target related state_dict
        model_state_dict = {
            k.replace("model_target.", ""): v
            for k, v in ckpt["state_dict"].items()
            if k.replace("model_target.", "") in self.model_target.state_dict()
        }
    ...
    def evaluate(self):
        eval_accuracy, eval_return, eval_length, eval_skill_ids, eval_video_path = self.agent.evaluate(
            self.actor.cuda(), self.model.cuda(), self.critic.cuda()
        )
        if len(eval_skill_ids) > 0:
            skill_id_counts = Counter(eval_skill_ids)
            skill_ids = {
