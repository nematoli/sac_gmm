...
@hydra.main(version_base="1.1", config_path="../config", config_name="kis_gmm_train")
def train(cfg: DictConfig) -> None:
    ...
    log_rank_0(f"Training a KIS-GMM skill with the following config:\n{OmegaConf.to_yaml(cfg)}")
    log_rank_0(print_system_env_info())
    chk = get_last_checkpoint(model_dir)

    # Load Model
    if chk is not None:
        model = KISGMM.load_from_checkpoint(chk.as_posix())
    else:
