...
from sac_gmm.models.gmm.utils.manifold_statistics import compute_frechet_mean, compute_weighted_frechet_mean
...
def manifold_gmm_em(
    ...
    for it in range(nb_iter_max):
        ...
        for k in range(nb_states):
            ...
            # Update covariances
            covariances[k] = (
                np.dot(xts_k.T, np.dot(np.diag(GAMMA2[:, k]), xts_k)) + np.eye(xts_k.shape[1]) * regularization_factor
            )
