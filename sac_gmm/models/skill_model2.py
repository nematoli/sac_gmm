...
from sac_gmm.models.sac.networks.state.encoder import Encoder
from sac_gmm.models.sac.networks.state.decoder import Decoder
...
class SkillModel2(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        super(SkillModel2, self).__init__()
        ...
        self.state_dim = encoder.embed_dim

        # Observation Encoder-Decoder
        env_ob_space = self.agent.env.get_observation_space()
        self.encoder = Encoder(encoder, env_ob_space["obs"], self.state_dim)
    ...
    def configure_optimizers(self):
        ...
        ae_optimizer = torch.optim.Adam(
            list(self.encoder.parameters()) + list(self.decoder.parameters()), lr=self.actor_lr
        )
        optimizers = [critic_optimizer, actor_optimizer, log_alpha_optimizer, ae_optimizer]
