...
from sac_gmm.models.skill_model2 import SkillModel2
...
class SACGMM2(SkillModel2):
    ...
    def loss(self, batch):
        critic_optimizer, actor_optimizer, alpha_optimizer, ae_optimizer = self.optimizers()
        recon_loss = self.compute_recon_loss(batch, ae_optimizer)
    ...
    def compute_recon_loss(self, batch, ae_optimizer):
        batch_obs = batch[0]
        state = self.encoder({"obs": batch_obs["obs"].to(dtype=torch.float32)})
        recon_obs = self.decoder(state)
        recon_loss = -recon_obs["obs"].log_prob(batch_obs["obs"]).mean()
