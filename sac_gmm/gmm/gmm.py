...
class ManifoldGMM(object):
    ...
    def save_params(self, filename="/gmm_params.npz"):
        np.savez(
            self.skills_dir + filename, gmm_means=self.means, gmm_covariances=self.covariances, gmm_priors=self.priors
        )
    ...
    def train(self, dataset):
        ...
        # K-Means
        km_means, km_assignments = manifold_k_means(self.manifold, self.data, nb_clusters=self.n_comp)
        # GMM
