...
class CalvinSkillEnv(PlayTableSimEnv):
    ...
    def get_observation_space():
        ...
        observation_space = {}
        # observation_space["position"] = gym.spaces.Box(low=np.array([0.3, -0.85, 0]), high=np.array([0.85, 0.85, 0.8]))
        observation_space["position"] = gym.spaces.Box(low=-np.ones(3), high=np.ones(3))
    ...
    def step(self, action):
        ...
        """
        # TODO: make this work with a more general action description
        coords = action[:3] * self.skill.dt
        orientation = np.zeros(3) * self.skill.dt
